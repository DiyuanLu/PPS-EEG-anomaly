...
def scan_animals_with_pretrained_model(args):
    ...
    del v_errors
    np.save(output_directory + 'valid_data' + '/whole_segment_v_errors.npy',
            whole_segment_v_errors)
    # whole_segment_v_errors = np.load(output_directory+'valid_data'+'/whole_segment_v_errors.npy')
    moving_average = pd.Series(whole_segment_v_errors).rolling(
        3600 * 12).mean()
    moving_std = pd.Series(whole_segment_v_errors).rolling(3600 * 12).std()
    fig = plt.figure(figsize=(20, 10))
    plt.scatter(np.arange(int(whole_segment_v_errors.shape[0])),
                whole_segment_v_errors, color='orange', label='errors',
                marker='.', s=1)
    plt.plot(moving_average, linewidth=2, color='black',
             label='moving average')
    plt.fill_between(moving_std.index, (moving_average - moving_std),
    ...
    plt.savefig(output_directory + 'v_whole_segment_errors.pdf')
    plt.close()
    ...
    plt.savefig(output_directory + 'whole_segment_errors.pdf')
    plt.close()

    th_whole_segment_t_errors = np.copy(whole_segment_t_errors)
    th_whole_segment_t_errors[th_whole_segment_t_errors < th99] = 0
    fig = plt.figure(figsize=(20, 10))
    plt.scatter(np.arange(len(th_whole_segment_t_errors)),
                th_whole_segment_t_errors, marker='.', color='orange', s=1)
    plt.hlines(th99, 0, len(whole_segment_t_errors), colors='green',
    ...
    plt.savefig(output_directory + 'th_t_whole_segment_errors.pdf')
    plt.close()
    ...
    fig = plt.figure(figsize=(20, 10))
    plt.scatter(np.arange(int(t_distances.shape[0])), t_distances,
    ...
    plt.savefig(output_directory + 't_distances.pdf')
    plt.close()
    ...
    for window_in_minutes in test_window_in_minutes:
        ...
        if np.any(frequency_v.values > th99_frequency):
            plt.axvline(np.where(frequency_v > th99_frequency)[0][0], c='r',
        ...
        plt.xticks(ticks, np.arange(0, len(ticks), 1))
        plt.tight_layout()
        plt.savefig(output_directory + 'frequency_v_' + str(
            window_in_minutes) + ' minutes_no.png')
        plt.savefig(output_directory + 'frequency_v_' + str(
            window_in_minutes) + ' minutes_no.pdf')
        plt.close()
