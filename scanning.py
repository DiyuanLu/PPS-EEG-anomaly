...
def scan_animals_with_pretrained_model(args):
    ...
    if args.LOO:
        ...
        val_files = get_data_files_from_folder(animal_path + '/BL/',
                                                 train_valid_split=False)
    ...
    np.savetxt(os.path.join(args.run_logdir,
               ...
               delimiter=",")
    np.savetxt(os.path.join(args.run_logdir,
                            "{}_picked_BL_files_{}.csv".format(args.LOO_animal, len(val_files))),
               np.array(val_files), fmt="%s",
               delimiter=",")
    train_set = v2_create_dataset(trained_files, batch_size=batch_size, shuffle=False,
                          n_sec_per_sample=args.n_sec_per_sample, sr=args.sampling_rate)
    val_set = v2_create_dataset(valid_files, batch_size=batch_size,
    ...
    total_num_batches_train = (len(val_files) * 720 * 5) // (args.n_sec_per_sample * batch_size)
    ...
    hour_span_train = (get_timestamp_from_file(os.path.basename(val_files[-1])) - get_timestamp_from_file(os.path.basename(val_files[0]))) / 3600
    ...
    compute_distros(val_set, output_directory + 'train_data', total_batch=total_num_batches_train, num2coll=total_num_batches_train//10)
...
def load_all_errors_and_distances(output_directory, name="train"):
    t_errors = np.load(output_directory + '{}_data'.format(name) + '/errors.npy')
    # t_probilities = np.load(output_directory+'train_data'+'/probilities.npy')
    t_distances = np.load(output_directory + '{}_data'.format(name) + '/distances.npy')
